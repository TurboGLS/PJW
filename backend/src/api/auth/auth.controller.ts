...
export const add = async (
  ...
  try {
    ...
    // Invio Email
    const verifyUrl = `https://pjw.onrender.com/verify-email?token=${newUser.verificationToken}`;
    ...
    const randomIban =
      "IT" + Math.floor(Math.random() * 1_000_000_000_000_0000);
    ...
    const newMovimentoAperturaConto = await movimentoContoSrv.aperturaConto(
      new mongoose.Types.ObjectId(contoCorrenteId)
    );
      ...
      try {
        ...
        if (!user.active) {
          res.status(403).json({
            ...
            message:
              "Devi prima verificare la tua email per attivare l'account.",
          });
          return;
        }
...
export const refresh = async (
  ...
  try {
    ...
    try {
      ...
      res
        .status(401)
        .json({ error: "RefreshTokenError", message: "InvalidToken" });
      return;
    }
    ...
    if (!match) {
      ...
      res
        .status(401)
        .json({ error: "RefreshTokenError", message: "Invalid Token" });
      return;
    }
