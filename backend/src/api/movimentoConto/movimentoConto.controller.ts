...
export class MovimentoContoController {
  public async getMovimentoContoById(
    req: Request,
    res: Response
  ): Promise<void> {
    try {
      ...
      console.error(
        "Errore nel controller getMovimentoContoById:",
        error.message
      );
      res.status(500).json({
        message: "Errore durante il recupero del movimento conto.",
        error: error.message,
      });
    }
  }

  public async getAllMovimentiConto(
    req: Request,
    res: Response
  ): Promise<void> {
    try {
      ...
      console.error(
        "Errore nel controller getAllMovimentiConto:",
        error.message
      );
      res.status(500).json({
        message: "Errore durante il recupero di tutti i movimenti conto.",
        error: error.message,
      });
    }
  }
  ...
  public async getLimitedMovimentiConto(
    req: Request,
    res: Response
  ): Promise<void> {
    try {
      ...
      if (isNaN(limit) || limit <= 0) {
        res.status(400).json({
          message:
            "Il parametro 'limit' deve essere un numero intero positivo.",
        });
        return;
      }
      ...
      if (!email) {
        res
          .status(400)
          .json({ message: "Il parametro 'email' è obbligatorio." });
        return;
      }

      const contoCorrente = await ContoCorrenteSrv.getContoCorrenteByEmail(
        email
      );
      ...
      if (!contoCorrente?.id) {
        res.status(404).json({
          message:
            "Impossible trovare un contoCorrente correlato alla email inserita",
        });
        return;
      }

      const movimenti = await movimentoContoService.getLimitedMovimentiConto(
        limit,
        contoCorrente.id
      );

      const lastSaldo = await movimentoContoService.getLastOperationByContoId(
        contoCorrente.id
      );
      ...
      console.error(
        "Errore nel controller getLimitedMovimentiConto:",
        error.message
      );
      res.status(500).json({
        message: "Errore durante il recupero dei movimenti conto limitati.",
        error: error.message,
      });
    }
  }
  ...
  public async getMovimentiByCategoria(req: Request, res: Response) {
    try {
      ...
      const categoryId = await movimentoContoService.getCategoryIdByName(
        categoryName,
        categoryType
      );
      ...
      const movimenti = await movimentoContoService.getMovimentiByCategoria(
        limit,
        contoCorrente.id,
        categoryId
      );
  ...
  public async postRicarica(req: Request, res: Response, next: NextFunction) {
    try {
      ...
      const lastMovimento =
        await movimentoContoService.getLastOperationByContoId(contoCorrente.id);
      ...
      const newMovimento = await movimentoContoService.ricaricaUscita(
        lastMovimento,
        importo,
        numeroTelefono
      );
  ...
  public async postPagamentoUtenze(
    req: Request,
    res: Response,
    next: NextFunction
  ) {
    try {
      ...
      if (!contoCorrente) {
        res
          .status(400)
          .json("Conto corrente mittente non trovato per l'email specificata");
        return;
      }
      if (!contoCorrente.id) {
      ...
      const newMovimento = await movimentoContoService.pagamentoUtenze(
        lastMovimento,
        importo
      );
      if (!newMovimento) {
  ...
  public async postPrelievo(req: Request, res: Response, next: NextFunction) {
    try {
      ...
      const newMovimento = await movimentoContoService.prelievoContanti(
        lastMovimento,
        importo
      );
  ...
  public async postVersamentoBancomat(
    req: Request,
    res: Response,
    next: NextFunction
  ) {
    try {
      ...
      const newMovimento = await movimentoContoService.versamentoBancomat(
        lastMovimento,
        importo
      );
      if (!newMovimento) {
  ...
  // Bonifico
  public async postBonificoUscita(
    req: Request,
    res: Response,
    next: NextFunction
  ) {
    try {
      ...
      const contoCorrenteMittente =
        await ContoCorrenteSrv.getContoCorrenteByEmail(email);
      ...
      if (!contoCorrenteMittente.id) {
        res
          .status(404)
          .json({ message: "Conto corrente ID del mittente non trovato" });
        await operationLogSrv.createLog(
          email!,
          ipAddress,
          "BONIFICO",
      ...
      if (!contoCorrenteDestinatario.id) {
        res
          .status(404)
          .json({ message: "Conto corrente ID del destinatario non trovato" });
        await operationLogSrv.createLog(
          email!,
          ipAddress,
          "BONIFICO",
      ...
      const lastMovimentoMittente =
        await movimentoContoService.getLastOperationByContoId(
          contoCorrenteMittente.id
        );
      const lastMovimentoDestinatario =
        await movimentoContoService.getLastOperationByContoId(
          contoCorrenteDestinatario.id
        );

      if (!lastMovimentoMittente) {
        res
          .status(404)
          .json({ message: "Ultimo movimento del mittente non trovato" });
        await operationLogSrv.createLog(
          email!,
          ipAddress,
          "BONIFICO",
      ...
      if (!lastMovimentoDestinatario) {
        res
          .status(404)
          .json({ message: "Ultimo movimento del destinatario non trovato" });
        await operationLogSrv.createLog(
          email!,
          ipAddress,
          "BONIFICO",
      ...
      const newMovimento = await movimentoContoService.bonificoUscita(
        lastMovimentoMittente,
        lastMovimentoDestinatario,
        importo,
        causale
      );
      ...
      if (!newMovimento) {
        res
          .status(500)
          .json({ message: "Errore nella creazione del movimento" });
        await operationLogSrv.createLog(
          email!,
          ipAddress,
          "BONIFICO",
  ...
  public async lastMovimentiBetweenDates(
    req: Request,
    res: Response,
    next: NextFunction
  ) {
    try {
      ...
      if (!email) {
        res
          .status(400)
          .json(
            `Impposibile recuperare le informazioni associate alla email ${email}`
          );
        return;
      }
      ...
      if (isNaN(startDate.getTime()) || isNaN(endDate.getTime())) {
        res.status(400).json({
          message:
            "Formato data non valido. Usa un formato ISO valido (es. YYYY-MM-DD).",
        });
        return;
      }

      const contoCorrenteId = await ContoCorrenteSrv.getContoCorrenteByEmail(
        email
      );
      ...
      if (!contoCorrenteId?.id) {
        res
          .status(400)
          .json(
            `Impossibile trovare il conto corrente associato alla email ${email}`
          );
        return;
      }

      const lastMovimentiDates =
        await movimentoContoService.getLimitedMovimentiBetweenDates(
          contoCorrenteId.id,
          startDate,
          endDate,
          limit
      ...
      if (!lastMovimentiDates) {
        res
          .status(400)
          .json(
            `Impossibile trovare gli ultimi movimenti tra data ${dataInizio} e ${dataFine}`
          );
        return;
      }
